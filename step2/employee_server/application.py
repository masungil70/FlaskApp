...
# 사진 저장을 위한 정적 파일 마운트
if not os.path.exists(config.PHOTOS_FOLDER): # 사진 폴더가 없으면 생성
    os.makedirs(config.PHOTOS_FOLDER)
app.mount("/uploads", StaticFiles(directory=config.PHOTOS_FOLDER), name="uploads") # /uploads 경로로 정적 파일 제공
...
def get_photo_url_for_fastapi(filename: str):
    return f"/uploads/{filename}" # 사진 URL 반환
...
async def save_employee(
    ...
    if photo and photo.filename != '': # 사진 파일이 제공되면
        ...
        if image_bytes:
            try:
                filename = util.random_hex_bytes(8) + '.png' # 고유한 파일 이름 생성
                filepath = os.path.join(config.PHOTOS_FOLDER, filename) # 파일 저장 경로
                with open(filepath, 'wb') as f: # 파일 쓰기
                    # 큰 파일의 경우 비동기 읽기/쓰기가 더 좋지만, 작은 이미지의 경우 동기 방식도 괜찮습니다.
                    f.write(image_bytes)
                key = filename # 저장된 파일 이름
            except Exception as e:
                raise HTTPException(status_code=500, detail=f"Could not save image: {e}") # 이미지 저장 실패 시 예외 발생
    ...
    if employee_id: # 기존 직원 업데이트
        ...
        if key:
            ...
            if old_employee and old_employee.object_key: # 이전 사진이 있으면
                try:
                    filepath = os.path.join(config.PHOTOS_FOLDER, old_employee.object_key) # 이전 사진 경로
                    if os.path.exists(filepath): # 파일이 존재하면 삭제
                        os.remove(filepath)
                except Exception as e:
                    print(f"Error deleting old photo file: {e}") # 오류 로깅 (계속 진행)
...
async def delete_employee_route(employee_id: int, current_user: str = Depends(get_current_user)):
    ...
    if employee.object_key:
        try:
            filepath = os.path.join(config.PHOTOS_FOLDER, employee.object_key) # 사진 파일 경로
            if os.path.exists(filepath): # 파일이 존재하면 삭제
                os.remove(filepath)
        except Exception as e:
            print(f"Error deleting photo file: {e}") # 오류 로깅 (계속 진행)
